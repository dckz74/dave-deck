  ...
  return {
    ...
    lastRoundWinner:
      gameState.lastRoundWinner === 'player'
        ? 'opponent'
        : gameState.lastRoundWinner === 'opponent'
          ? 'player'
...
function flipRoundResult(roundResult) {
  ...
  return {
    ...
    lifeLost:
      roundResult.lifeLost === 'player'
        ? 'opponent'
        : roundResult.lifeLost === 'opponent'
          ? 'player'
...
io.on('connection', socket => {
  ...
  socket.on('join-session', async data => {
    try {
      const { sessionId, playerName } =
        typeof data === 'string' ? { sessionId: data, playerName: null } : data
  ...
  socket.on('game-action', async actionData => {
    ...
    if (currentGameState && currentGameState.phase === 'playing') {
      ...
      if (shouldHostAct !== hostIsActing) {
        console.log(
          `🚫 Turn validation failed for ${actingPlayer?.name} (${isHost ? 'host' : 'guest'}). Server expects ${shouldHostAct ? 'host' : 'guest'} to act, but ${hostIsActing ? 'host' : 'guest'} acted.`
        )
    ...
    if (otherPlayer) {
      const stateForOtherPlayer = otherPlayer.isHost
        ? canonicalState
        : flipGameState(canonicalState)

      console.log(
        `Game action in session ${sessionId}: ${actionData.action} by ${actingPlayer?.name} (${isHost ? 'host' : 'guest'})`
      )
  ...
  socket.on('round-transition', async data => {
    ...
    session.players.forEach(player => {
      ...
      const starterFromHostPerspective =
        nextStarter === 'player' ? hostPlayer?.name : guestPlayer?.name
    ...
    console.log(
      `Round ${session.gameMetadata.roundNumber} started in session ${sessionId}, starting player: ${nextStarter}`
    )
  })
    ...
    session.players.forEach(player => {
      ...
      if ((isActingPlayerHost && !isHost) || (!isActingPlayerHost && isHost)) {
        ...
        gameResultForPlayer = {
          ...gameResult,
          gameWinner:
            gameResult.gameWinner === 'player'
              ? 'opponent'
              : gameResult.gameWinner === 'opponent'
                ? 'player'
  ...
  socket.on('update-player-name', async data => {
    ...
    if (player) {
      ...
      console.log(
        `Player ${socket.id} updated name from "${oldName}" to "${player.name}" in session ${sessionId}`
      )
